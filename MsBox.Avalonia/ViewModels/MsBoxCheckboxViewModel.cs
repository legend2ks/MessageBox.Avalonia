...
public class MsBoxCheckboxViewModel : AbstractMsBoxViewModel, ISetFullApi<MessageBoxCheckboxResult>
{
    public readonly ClickEnum _enterDefaultButton;
    public readonly ClickEnum _escDefaultButton;
    private IFullApi<MessageBoxCheckboxResult> _fullApi;

    public MsBoxCheckboxViewModel(MessageBoxCheckboxParams @params) :
        base(@params, @params.Icon)
    {
        ...
        CheckboxText = @params.CheckboxText;
        IsCheckboxChecked = @params.CheckboxDefaultState;
    }

    public void SetFullApi(IFullApi<MessageBoxCheckboxResult> fullApi)
    {
        _fullApi = fullApi;
    ...
    public string CheckboxText { get; internal set; }
    ...
    public async void ButtonClick(string parameter)
    {
        ...
        {
            var result = new MessageBoxCheckboxResult(IsCheckboxChecked,
                (ButtonResult)Enum.Parse(typeof(ButtonResult), parameter.Trim(), true));
            _fullApi.SetButtonResult(result);
            _fullApi.Close();
        });
    }
    ...
    public async void ButtonClick(ButtonResult buttonResult)
    {
        await Dispatcher.UIThread.InvokeAsync(() =>
        {
            var result = new MessageBoxCheckboxResult(IsCheckboxChecked, buttonResult);
            _fullApi.SetButtonResult(result);
            _fullApi.Close();
        });
    }
